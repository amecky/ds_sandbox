...
ParticlePluginTest::ParticlePluginTest(const plugin_registry& registry, SpriteBatchBuffer* buffer) : _sprites(buffer) {
	...
	_numParticles = 64;
	_emissionTime = 2.0f;
	// G:\NB\ds_sandbox\plugins\particle_manager\x64\Debug
	//  G:\NB\plugins\particle_manager\x64\Debug\particle_manager.dll
...
void ParticlePluginTest::render() {
	...
	while (cnt < _data->index) {
		_sprites->add(*pos, ds::vec4(200, 0, 20, 20), *scales, *r, *c);
		++cnt;
		++pos;
		++scales;
		++r;
