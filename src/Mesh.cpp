...
bool starts_with(const char* text, const std::string& line) {
	int l = strlen(text);
	if (l > line.length()) {
		return false;
	}
...
bool convert(const char* line, int offset, float* data, int max) {
	const char* ll = line;
	ll += offset;
	char *out;
	for (int i = 0; i < max; ++i) {
...
ds::vec3 convert_to_vec3(const char* line, int offset) {
	float v[3];
...
ds::vec2 convert_to_vec2(const char* line, int offset) {
	const char* ll = line;
	ll += offset;
	char *out;
	ds::vec2 v;
	for (int i = 0; i < 2; ++i) {
		v.data[i] = my_strtof(ll, &out);
		ll = out;
		++ll;
	}
	return v;
...
int parseF(const char* cmd, int* v, int max) {
	const char* p = cmd;
	int cnt = 0;
	for (int i = 0; i < max; ++i) {
		v[i] = -1;
...
int Mesh::loadMaterial(const char* dir, const char* name, color_definition* defs, int max) {
	char fileName[256];
...
void Mesh::loadObj(const char* dir, const char* name) {
	char fileName[256];
	sprintf_s(fileName, "%s\\%s", dir, name);
	std::ifstream file(fileName);
	std::string curline;
	...
	while (std::getline(file, curline)) {
		if (starts_with("#", curline)) {
			...
			const char* name = curline.c_str();
			name += 7;
			num_materials = loadMaterial(dir, name, defs, 32);
		}
	...
	addStream(AT_VERTEX, (float*)verts, total, 3);
	addStream(AT_COLOR, (float*)colors, total, 4);
	addStream(AT_NORMAL, (float*)ns, total, 3);
}